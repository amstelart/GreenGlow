...
(function($, root) {
  ...
  if(typeof root.videoResize == 'undefined') {
      /**
       * Ресайз видео
       *
      ...
      root.videoResize = function(videoContainer, video) {
          ...
          if (videoContainerWidth / k >= videoContainerHeight) {
              video
                  ...
                  .height(videoContainerWidth / k)
                  .css('margin-top', -(video.height() - videoContainerHeight) / 2);
          } else {
              video
                  .removeAttr('style')
                  .height(videoContainerHeight)
                  .width(videoContainerHeight * k)
                  .css('margin-left', -(video.width() - videoContainerWidth) / 2);
          };
      };
  }

  // по готовности документа ресазйм видео и вешаем бинды на ресайз окна
  $(document).ready(function () {
      root.videoResize(videoContainer, video);
